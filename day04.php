...
function isValid(string $p): bool
{
    $words = explode(' ', $p);
    foreach ($words as $i => $w1) {
        $s1 = sortLetter($w1);
        foreach ($words as $j => $w2) {
            $s2 = sortLetter($w2);
            if ($i !== $j && $s1 === $s2) {
                return false;
            }
...
function sortLetter(string $word): string
{
    $letters = preg_split('/(.)/', $word, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
    sort($letters);
