...
function solve1(string $input): string
{
    $nodes = parse($input);
    $allChildren = [];
    foreach ($nodes as $n) {
        $allChildren = array_merge($allChildren, $n['c']);
    }

    $notChild = array_diff(array_keys($nodes), $allChildren);

    return reset($notChild);
}
...
function solve2(string $input): int
{
    ...
    $unbalanced = findUnbalanced($nodes, $parent);

    $otherChildren = array_diff($nodes[$nodes[$unbalanced]['p']]['c'], [$unbalanced]);
    $otherChild = reset($otherChildren);

    return $nodes[$unbalanced]['w'] + weight($nodes, $otherChild) - weight($nodes, $unbalanced);
}

function findUnbalanced(array $nodes, string $name): ?string
{
    $weights = [];
    foreach ($nodes[$name]['c'] as $c) {
        $weight = weight($nodes, $c);
...
function weight(array $nodes, string $name): int
{
    $weight = $nodes[$name]['w'];
    foreach ($nodes[$name]['c'] as $c) {
        $weight += weight($nodes, $c);
...
{
    preg_match_all('~(\w+) \((\d+)\)( -> (.*))?~', $input, $m);

    $nodes = [];
    foreach ($m[1] as $i => $name) {
        $nodes[$name] = [
            'w' => (int) $m[2][$i],
            'c' => preg_split('~, ~', $m[4][$i], -1, PREG_SPLIT_NO_EMPTY),
        ];
    }
...
$input = 'mmqyju (156) -> rjzvwv, noybkx
...
assertSame('tknk', solve1($sample1));
var_dump(solve1($input));

assertSame(60, solve2($sample1));
