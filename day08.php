...
{
    preg_match_all('/((\w+) (dec|inc)+ -?\d+) (if (.*))/', $input, $m);

    $init = '';
    $c = '';
    $final = [];
    foreach ($m[2] as $i => $registry) {
        $init .= '$' . $registry . ' = 0;' . PHP_EOL;
        $c .= 'if ($' . $m[5][$i] . ') ' . '$' . $registry . ' = $' . str_replace([' inc ', ' dec '], [' + ', ' - '], $m[1][$i]) . ';' . PHP_EOL;
        $final[] = '$' . $registry;
    }
    $final = 'return max([' . implode(', ', $final) . ']);' . PHP_EOL;
...
function solve2(string $input): int
{
    ...
    foreach ($m[2] as $i => $registry) {
        ...
        $c .= 'if ($' . $m[5][$i] . ') ' . '$' . $registry . ' = $' . str_replace([' inc ', ' dec '], [' + ', ' - '], $m[1][$i]) . ';' . PHP_EOL;
        $final[] = '$' . $registry;
        $c .= '$allTimeMaximum[] = max([' . implode(', ', $final) . ']);' . PHP_EOL;
    }

    $final = 'return max($allTimeMaximum);' . PHP_EOL;
    $code = $init . $c . $final;
...
assertSame(1, solve1($sample1));
var_dump(solve1($input));

assertSame(10, solve2($sample1));
